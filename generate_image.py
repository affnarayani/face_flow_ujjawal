...
def can_run_image_script() -> tuple:
    print("[STEP] Checking status in ujjawal_fb_topics.json...", flush=True)
    topics_file = Path("ujjawal_fb_topics.json")
    if not topics_file.exists():
        print("[INFO] 'ujjawal_fb_topics.json' nahi mili. Execution stopped.", flush=True)
        return False, None
...
def update_image_status_in_json(topic_text: str):
    print("[STEP] Updating image status in ujjawal_fb_topics.json...", flush=True)
    topics_file = Path("ujjawal_fb_topics.json")
    if not topics_file.exists():
        return
    ...
    print("[OK] Image status successfully updated (image_generated=True) in ujjawal_fb_topics.json", flush=True)
...
def run():
    ...
    try:
        ...
        prompt = ("""
        Create image for a Facebook post with title: "{post_title}". Image must in the ratio 1:1. This will be a hero image to this Facebook post. Image must be engaging. The core idea of the image must revolve around these keywords: {post_keywords}. Output only the final image.
        """)
